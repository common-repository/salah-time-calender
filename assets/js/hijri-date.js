...
function madinaHijriCalendar(adjust){
	var today = new Date();
	if(adjust) {
	...
	var wd;
	if(adjust) {
		wd = madinagmod(jd+1-adjust,7)+1;
	} else {
		wd = madinagmod(jd+1,7)+1;
	}
...
function madinaHijriDate(adjustment) {
	...
	var iDate = madinaHijriCalendar(adjustment);
